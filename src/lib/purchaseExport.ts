...
async function save(wb: import("xlsx").WorkBook, filename: string) {
  const XLSX = await import("xlsx");
  XLSX.writeFile(wb, filename);
}

/** Export a list of purchases (one row per purchase) to .xlsx */
export async function exportPurchasesToExcel(purchases: PurchaseExportRow[], filename?: string) {
  const XLSX = await import("xlsx");
  ...
  await save(wb, filename || `purchases-${format(new Date(), "yyyy-MM-dd")}.xlsx`);
}

/** Export a single purchase document (header + line items) to .xlsx */
export async function exportPurchaseDocumentToExcel(
  ...
  await save(wb, `purchase-${ref}.xlsx`);
}
