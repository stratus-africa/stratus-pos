...
export async function handleStartSupportSession(
  ...
  if (authTargetError || !authTarget?.user?.email)
    throw new Error("Tenant administrator authentication account could not be resolved");
  if (authTarget.user.email.toLowerCase() !== profile.email.toLowerCase())
    throw new Error("Tenant administrator identity mismatch");
