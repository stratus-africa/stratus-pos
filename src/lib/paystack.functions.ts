...
export const paystackInitialize = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<PaystackInitializeResult> => {
    ...
    const amountKes = data.interval === "monthly" ? Number(pkg.monthly_price_kes) : Number(pkg.yearly_price_kes);

    if (!amountKes || amountKes <= 0) {
      throw new Error("This plan has no KES price configured. Ask the platform admin to set it.");
    }

    let planCode = data.interval === "monthly" ? pkg.paystack_plan_code_monthly : pkg.paystack_plan_code_yearly;
...
export const paystackListTransactions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<PaystackListTransactionsResult> => {
    // This read only needs the authenticated user's identity/RLS. Do not
    // require the Supabase service-role secret just to view Paystack
    // transactions in Super Admin. The Paystack secret remains server-only
    // and is used exclusively by paystackFetch below.
    const { data: isSA, error: roleError } = await context.supabase.rpc("is_super_admin", {
