...
function EmployeeDialog({
  ...
  const [form, setForm] = useState<Partial<Employee>>(
    editing ?? { employment_type: "full_time", status: "active", basic_salary: 0 },
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <Label>NSSF #</Label>
            <Input value={form.nssf_no ?? ""} onChange={(e) => setForm({ ...form, nssf_no: e.target.value })} />
          </div>
          <div>
            <Label>NHIF / SHIF #</Label>
            <Input value={form.nhif_no ?? ""} onChange={(e) => setForm({ ...form, nhif_no: e.target.value })} />
          </div>
          <div>
...
function EmployeesTab() {
  ...
  const rows = (query.data ?? []).filter((e) =>
    [e.full_name, e.employee_number, e.job_title, e.department]
      .filter(Boolean)
              ...
              {query.isLoading ? (
                ...
                rows.map((e) => (
                  <TableRow key={e.id} className="odd:bg-muted/30">
                    ...
                    <TableCell>
                      <Badge variant="secondary">{e.employment_type.replace("_", " ")}</Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    {canManage && (
                      <TableCell className="text-right">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            if (confirm(`Remove ${e.full_name}?`)) remove.mutate(e.id);
                          }}
                        >
            ...
            {(query.data ?? []).map((t) => (
              <TableRow key={t.id}>
                ...
                <TableCell className="text-xs capitalize text-muted-foreground">
                  {t.accrual_frequency}
                  {t.carry_forward_limit > 0 ? ` · CF ${t.carry_forward_limit}` : ""}
                </TableCell>
                <TableCell>{t.is_paid ? "Yes" : "No"}</TableCell>
                {canManage && (
                  <TableCell className="text-right">
                    <Button size="icon" variant="ghost" onClick={() => start(t)}>
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => {
                        if (confirm("Delete leave type?")) remove.mutate(t.id);
                      }}
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
          ...
          <div className="space-y-3">
                ...
                <SelectContent>
                  {LEAVE_ICON_OPTIONS.map((o) => (
                    <SelectItem key={o.value} value={o.value}>
                      <div className="flex items-center gap-2">
                        <LeaveIcon name={o.value} /> {o.label}
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label>Accrual</Label>
                <Select
                  value={form.accrual_frequency ?? "yearly"}
                  onValueChange={(v: any) => setForm({ ...form, accrual_frequency: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, carry_forward_limit: Number(e.target.value) })}
              />
              <p className="text-xs text-muted-foreground mt-1">
                Max unused days that roll to next year (0 = no carry-forward).
              </p>
            </div>
            <div className="flex items-center gap-2">
            ...
            {(query.data ?? []).slice(0, 8).map((a) => {
              ...
              return (
                <TableRow key={a.id}>
                  ...
                  <TableCell
                    className={`text-right font-medium ${a.delta >= 0 ? "text-emerald-600" : "text-destructive"}`}
                  >
                    {a.delta > 0 ? "+" : ""}
                    {a.delta}
                  </TableCell>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3 text-xs">
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-emerald-100 border border-emerald-300"></span>Approved
          </span>
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-amber-100 border border-amber-300"></span>Pending
          </span>
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-rose-100 border border-rose-300"></span>Rejected
          </span>
        </div>
                ...
                <TableBody>
                  ...
                  {rows.map((r) => (
                    <TableRow key={r.id} className="odd:bg-muted/30">
                      ...
                      <TableCell>
                        <Badge
                          className={r.status === "approved" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : ""}
                          variant={
                            r.status === "approved"
                              ? "default"
                              : r.status === "rejected"
...
/* ---------------- Payslip PDF ---------------- */
async function generatePayslipPdf(payslip: any, businessName: string | undefined) {
  const [{ default: jsPDF }, { default: autoTable }] = await Promise.all([import("jspdf"), import("jspdf-autotable")]);
  const doc = new jsPDF();
  doc.setFontSize(16);
  doc.text(businessName || "Payslip", 14, 20);
  doc.setFontSize(11);
  doc.text(`Payslip — ${MONTHS[payslip.period_month - 1]} ${payslip.period_year}`, 14, 28);
  doc.setFontSize(10);
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        ...
        { content: Number(payslip.gross_pay).toLocaleString(), styles: { fontStyle: "bold" } },
      ],
    ],
  });
  autoTable(doc, {
    ...
    body: [
      ...((payslip.deductions ?? []).length === 0
        ? [["None", "0"]]
        : (payslip.deductions ?? []).map((d: any) => [d.label, Number(d.amount).toLocaleString()])),
      [
        { content: "Total Deductions", styles: { fontStyle: "bold" } },
        { content: Number(payslip.total_deductions).toLocaleString(), styles: { fontStyle: "bold" } },
      ],
    ],
  });
  autoTable(doc, {
    ...
    body: [
      [
        { content: "NET PAY", styles: { fontStyle: "bold", fontSize: 13 } },
        {
          content: `KES ${Number(payslip.net_pay).toLocaleString()}`,
          styles: { fontStyle: "bold", fontSize: 13, halign: "right" },
        },
      ],
    ],
  });
  ...
  doc.save(
    `Payslip-${payslip.employees?.full_name ?? "employee"}-${payslip.period_year}-${String(payslip.period_month).padStart(2, "0")}.pdf`,
  );
}
...
function PayslipDialog({
  ...
  const gross =
    ...
    (form.allowances ?? []).reduce((s: number, a: any) => s + Number(a.amount || 0), 0);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editing ? "Edit Payslip" : "New Payslip"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          {(["allowances", "deductions"] as const).map((key) => (
            <div key={key}>
              <div className="flex items-center justify-between mb-2">
                ...
                <Button size="sm" variant="outline" onClick={() => addLine(key)}>
                ...
                {(form[key] ?? []).map((l: any, i: number) => (
                  <div key={i} className="flex gap-2">
                    <Input
                      placeholder="Label"
                      value={l.label}
                      onChange={(e) => updateLine(key, i, "label", e.target.value)}
                    />
                    <Input
                      type="number"
                      placeholder="Amount"
                      className="w-40"
                      value={l.amount}
                      onChange={(e) => updateLine(key, i, "amount", e.target.value)}
                    />
                    <Button size="icon" variant="ghost" onClick={() => removeLine(key, i)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                ))}
          ...
          <div className="rounded border p-3 space-y-1 bg-muted/30 text-sm">
            <div className="flex justify-between">
              <span>Gross Pay</span>
              <span className="font-medium">{KES(gross)}</span>
            </div>
            <div className="flex justify-between">
              <span>Total Deductions</span>
              <span className="font-medium">{KES(totalDed)}</span>
            </div>
            <div className="flex justify-between text-base pt-1 border-t">
              <span className="font-semibold">Net Pay</span>
              <span className="font-bold text-primary">{KES(net)}</span>
            </div>
          </div>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div id="payslip-print" className="space-y-3 text-sm">
          ...
          <div className="grid grid-cols-2 gap-1">
            ...
            <div className="text-right font-medium">{payslip.employees?.full_name}</div>
            <div className="text-muted-foreground">Status</div>
            <div className="text-right">{payslip.status}</div>
          </div>
          <div className="border-t pt-2">
            <div className="font-semibold mb-1">Earnings</div>
            <div className="flex justify-between">
              <span>Basic Salary</span>
              <span>{KES(payslip.basic_salary)}</span>
            </div>
            {(payslip.allowances ?? []).map((a: any, i: number) => (
              <div key={i} className="flex justify-between">
                <span>{a.label}</span>
                <span>{KES(a.amount)}</span>
              </div>
            ))}
            <div className="flex justify-between border-t mt-1 pt-1 font-medium">
              <span>Gross Pay</span>
              <span>{KES(payslip.gross_pay)}</span>
            </div>
          </div>
          <div>
            ...
            <div className="flex justify-between border-t mt-1 pt-1 font-medium">
              <span>Total Deductions</span>
              <span>{KES(payslip.total_deductions)}</span>
            </div>
          </div>
          <div className="flex justify-between border-t-2 pt-2 text-base font-bold">
            <span>Net Pay</span>
            <span className="text-primary">{KES(payslip.net_pay)}</span>
          </div>
        ...
        <DialogFooter>
          ...
          <Button variant="outline" onClick={() => generatePayslipPdf(payslip, business?.name)}>
    ...
    <div className="space-y-4">
      {canManage && (
        <div className="flex justify-end">
          ...
          >
            <Plus className="h-4 w-4 mr-1" /> New Payslip
          </Button>
        </div>
      )}
      <Card>
            ...
            <TableBody>
              ...
              {rows.map((p: any) => (
                <TableRow key={p.id} className="odd:bg-muted/30">
                  ...
                  <TableCell className="text-right space-x-1">
                    <Button size="icon" variant="ghost" onClick={() => setViewing(p)} title="View">
                      <FileText className="h-4 w-4" />
                    </Button>
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => generatePayslipPdf(p, business?.name)}
                      title="Download PDF"
                    >
                    ...
                    {canManage && p.status === "draft" && (
                      <>
                        ...
                        <Button size="sm" variant="outline" onClick={() => issue.mutate(p.id)}>
                          Issue
                        </Button>
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => {
                            if (confirm("Delete payslip?")) remove.mutate(p.id);
                          }}
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
...
function PayrollRunDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const submit = async () => {
    if (!bankAccountId) {
      alert("Choose a bank account to pay from");
      return;
    }
    if (chosen.length === 0) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <Select value={bankAccountId} onValueChange={setBankAccountId}>
                <SelectTrigger>
                  <SelectValue placeholder="Choose bank/cash" />
                </SelectTrigger>
                <SelectContent>
                  {(banks ?? []).map((b) => (
                    <SelectItem key={b.id} value={b.id}>
                      {b.name} ({KES(b.balance)})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                  ...
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="text-right">
                      <Input
                        ...
                        value={salaryOverrides[e.id] ?? e.basic_salary}
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <Wallet className="h-5 w-5" /> Payroll Runs
          </h3>
