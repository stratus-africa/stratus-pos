...
const Reports = () => {
  ...
  // Profiles lookup for resolving cashier UUIDs → names in Sales by Cashier
  const profilesQuery = useQuery({
    queryKey: ["report-profiles", business?.id],
    queryFn: async () => {
    ...
    enabled: !!business && activeTab === "sales_by_cashier",
  });
  ...
  const featureReport = useQuery({
    ...
    queryFn: async () => {
      ...
      if (k === "sales_by_cashier") {
        const profileMap = new Map((profilesQuery.data || []).map((p) => [p.id, p.full_name || "Unknown"]));
        const m = new Map<string, any>();
        for (const r of salesReport.data || []) {
          const id = r.created_by || "Unknown";
          const name = profileMap.get(id) || id.slice(0, 8) || "Unknown";
          const x = m.get(id) || { cashier: name, sales: 0, revenue: 0 };
          x.sales++;
          x.revenue += Number(r.total || 0);
          m.set(id, x);
        }
      ...
      if (k === "purchases_by_supplier") {
        const m = new Map<string, any>();
        for (const p of purchasesReport.data || []) {
          const supplierName = p.suppliers?.name || "Unknown Supplier";
          const locationName = p.locations?.name || "Unknown Location";
          const key = `${supplierName}__${locationName}`;
          const x = m.get(key) || { supplier: supplierName, location: locationName, orders: 0, total: 0 };
          x.orders++;
      ...
      if (k === "purchase_returns") {
        return (purchasesReport.data || [])
          .filter((p: any) => p.status === "returned" || p.status === "return" || p.status === "partial_return")
          .map((p: any) => ({
            date: new Date(p.created_at).toLocaleDateString(),
            invoice: p.invoice_number || "—",
            supplier: p.suppliers?.name || "Unknown Supplier",
            location: p.locations?.name || "Unknown Location",
            total: p.total,
            status: p.status,
            payment_status: p.payment_status,
          }));
      }
      if (k === "purchases") return purchasesReport.data || [];
