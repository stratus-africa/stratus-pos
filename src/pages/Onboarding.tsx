...
const RegistrationForm = ({ hasUser }: { hasUser: boolean }) => {
  ...
  const submitRegistration = async () => {
    ...
    // Link profile even if the auth trigger did not create a row yet.
    const { error: profileUpsertError } = await supabase
      .from("profiles")
      .upsert({ id: currentUser.id, business_id: businessId, full_name: contactPerson.trim() }, { onConflict: "id" });

    if (profileUpsertError) {
      toast.error(profileUpsertError.message);
      setSubmitting(false);
      return;
    }
    ...
    const { error: roleInsertError } = await supabase
      .from("user_roles")
      .upsert(
        { user_id: currentUser.id, business_id: businessId, role: "admin" },
