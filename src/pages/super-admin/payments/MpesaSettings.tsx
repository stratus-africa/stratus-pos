...
import { ChevronLeft, Loader2, Check, ExternalLink, Users } from "lucide-react";
...
import SuperAdminMpesaTenants from "@/pages/super-admin/SuperAdminMpesaTenants";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Link
          to="/super-admin/settings/payments/mpesa?view=tenants"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Daraja account</CardTitle>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={cfg.callback_base_url ?? ""}
              onChange={(e) => set("callback_base_url", e.target.value)}
              placeholder="https://yourdomain.com"
            />
            <p className="text-xs text-muted-foreground">
              Daraja will POST to{" "}
              <code className="text-xs">
                {(cfg.callback_base_url || "https://yourdomain.com").replace(/\/$/, "")}/mpesa/callback
              </code>
              .
            </p>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Secret keys</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Daraja credentials (<code className="text-xs">MPESA_CONSUMER_KEY</code>,{" "}
            <code className="text-xs">MPESA_CONSUMER_SECRET</code>,<code className="text-xs"> MPESA_PASSKEY</code>,{" "}
            <code className="text-xs">MPESA_B2C_*</code>) are stored securely and never exposed to the browser.
          </p>
          <a
...
export default function MpesaSettings() {
  ...
  return searchParams.get("view") === "tenants" ? <SuperAdminMpesaTenants /> : <MpesaSettingsForm />;
}
