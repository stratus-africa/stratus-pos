...
export default function SuperAdminPackages() {
  ...
  const fetchAll = async () => {
    ...
    try {
      // IMPORTANT: this is the Superadmin plan catalogue. It must load ALL
      // plans, including private and inactive plans. Public pricing visibility
      // is handled separately by get_public_subscription_packages().
      const [pkgRes, featRes, subsRes] = await Promise.all([
        supabase.from("subscription_packages").select("*").order("sort_order", { ascending: true }),
        supabase.from("package_features").select("*"),
        supabase.from("subscriptions").select("product_id, status"),
      ]);

      if (pkgRes.error) {
        console.error("Failed to load Superadmin subscription plans:", pkgRes.error);
        throw new Error(pkgRes.error.message);
      }

      // Feature/subscription failures must not make the plan catalogue
      // disappear. The plans themselves are the primary data for this page.
      if (featRes.error) {
        console.warn("Failed to load plan features:", featRes.error);
      }
      if (subsRes.error) {
        console.warn("Failed to load subscription counts:", subsRes.error);
      }

      setPackages((pkgRes.data as PackageData[]) || []);
      setFeatures(featRes.error ? [] : (featRes.data as PackageFeature[]) || []);
      ...
      setFeatures([]);
      setSubCounts({});
      setLoadError(error?.message || "Failed to load subscription plans.");
    } finally {
      setLoading(false);
    }
  };
