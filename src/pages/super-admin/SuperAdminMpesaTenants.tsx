...
import { setMpesaCredentials, deleteMpesaCredentials, testMpesaCredentials, testMpesaStk } from "@/lib/mpesaCredentials.functions";
...
export default function SuperAdminMpesaTenants() {
  ...
  const testStkFn = useServerFn(testMpesaStk);
  ...
  const testStk = async () => {
    ...
    setTesting(true);
    try {
      const result = await testStkFn({ data: { business_id: selected.id, phone_number: testPhone, amount: Number(testAmount), environment: selected.mpesa_environment === "live" ? "live" : "sandbox" } });
      if (result.ok) toast.success("STK test sent", { description: `CheckoutRequestID: ${result.checkout_request_id}` });
      else toast.error("STK test failed");
