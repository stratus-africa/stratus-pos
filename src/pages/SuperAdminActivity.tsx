...
import { Activity, ChevronLeft, ChevronRight, FileText, Search, User } from "lucide-react";
...
export default function SuperAdminActivity() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" /><h1 className="text-2xl font-bold">Audit Log</h1></div>
        <p className="text-sm text-muted-foreground mt-1">Immutable platform activity across all tenants and privileged actions.</p>
      </div>

      <Card>
        <CardContent className="p-4 flex flex-col lg:flex-row gap-3">
          <div className="relative flex-1 min-w-0"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Search action, tenant, user, entity…" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center justify-between"><span className="flex items-center gap-2"><Activity className="h-4 w-4" /> Activity</span><span className="text-xs font-normal text-muted-foreground">{filtered.length.toLocaleString()} events</span></CardTitle></CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-y bg-muted/30 text-left"><th className="px-4 py-3 font-medium">Time</th><th className="px-4 py-3 font-medium">Action</th><th className="px-4 py-3 font-medium">Tenant</th><th className="px-4 py-3 font-medium">Actor</th><th className="px-4 py-3 font-medium">Entity</th><th className="px-4 py-3" /></tr></thead><tbody>
            {loading ? <tr><td colSpan={6} className="p-10 text-center text-muted-foreground">Loading audit events…</td></tr> : pageRows.length === 0 ? <tr><td colSpan={6} className="p-10 text-center text-muted-foreground">No audit events match your filters.</td></tr> : pageRows.map((r) => <tr key={r.id} className="border-b hover:bg-muted/20 cursor-pointer" onClick={() => setSelected(r)}><td className="px-4 py-3 whitespace-nowrap"><div>{format(new Date(r.created_at), "MMM d, yyyy HH:mm")}</div><div className="text-[11px] text-muted-foreground">{formatDistanceToNow(new Date(r.created_at), { addSuffix: true })}</div></td><td className="px-4 py-3"><Badge variant="outline" className="font-mono text-[11px]">{r.action}</Badge><div className="text-xs text-muted-foreground mt-1 max-w-xs truncate">{r.description || "—"}</div></td><td className="px-4 py-3 font-medium">{tenantMap.get(r.business_id) || "Unknown tenant"}</td><td className="px-4 py-3"><div className="flex items-center gap-1.5"><User className="h-3.5 w-3.5 text-muted-foreground" />{r.user_name || r.user_email || "System"}</div></td><td className="px-4 py-3 text-xs text-muted-foreground">{r.entity_type || "—"}{r.entity_id ? <div className="font-mono truncate max-w-32">{r.entity_id}</div> : null}</td><td className="px-4 py-3 text-right">›</td></tr>)}
          </tbody></table></div>
