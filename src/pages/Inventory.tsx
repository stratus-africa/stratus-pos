...
const Inventory = () => {
  ...
  const canViewBatches = multiLocationEnabled && batchTrackingEnabled && hasPermission("inventory.view_batches");
  ...
  const handleAdjust = async (data: AdjustStockSubmit) => {
    ...
    // The currently deployed inventory_control_request RPC accepts the
    // canonical reasons Issue / Write-off / Adjustment. Keep the user's
    // descriptive Stock Adjustment reason in notes until the corresponding
    // database migration is deployed.
    const controlReason = data.reason === "Issue" ? "Issue" : data.reason === "Write-off" ? "Write-off" : "Adjustment";

    const preservedReason = data.reason && data.reason !== controlReason ? `Adjustment reason: ${data.reason}` : null;

    const combinedNotes = [preservedReason, data.notes?.trim()].filter(Boolean).join("\n");
  ...
  return (
    <div className="space-y-4">
      ...
      <StockAdjustmentDialog
        ...
        isLoading={adjustmentSubmitting || createPurchase.isPending}
      />
