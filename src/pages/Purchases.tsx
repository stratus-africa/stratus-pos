...
const Purchases = () => {
  ...
  const editPayment = (payment: any) => {
    const amount = window.prompt("Payment amount (KES)", String(Number(payment.amount || 0)));
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
              ...
              <Button
                ...
                onClick={() =>
                  exportPurchasesToExcel(
                    selectedPurchases,
                    `purchases-selected-${format(new Date(), "yyyy-MM-dd")}.xlsx`,
                  )
                }
              >
                <FileSpreadsheet className="mr-2 h-4 w-4" /> Export Excel
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This is permanent and cannot be restored. Received stock, stock movements, reconciliation
                        entries, linked payments and accounting postings will be removed.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                  ...
                  {payments.length === 0 ? (
                    ...
                    payments.map((p) => (
                      ...
                      >
                        ...
                        <TableCell>
                          {p.purchases?.invoice_number || (p.purchase_id ? p.purchase_id.slice(0, 8) : "—")}
                        </TableCell>
                        ...
                        <TableCell onClick={(event) => event.stopPropagation()}>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setViewingPayment(p)}>
                                <Eye className="mr-2 h-4 w-4" /> View
                              </DropdownMenuItem>
                              {canEdit && (
                                <DropdownMenuItem onClick={() => editPayment(p)}>
                                  <Pencil className="mr-2 h-4 w-4" /> Edit
                                </DropdownMenuItem>
                              )}
                              {canDelete && (
      ...
      <AlertDialog
        ...
        onOpenChange={(o) => {
          if (!o) setPurchaseToDelete(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Permanently delete purchase {purchaseToDelete?.invoice_number || purchaseToDelete?.id.slice(0, 8)}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This is permanent — the purchase cannot be restored afterwards.
              {purchaseToDelete?.status === "received"
                ? " Stock added by this purchase will be removed from inventory."
                : ""}
              {
              ...
              {purchaseToDelete?.status !== "cancelled" &&
                " If you only want to undo the stock effect, use Cancel Purchase from the edit page instead."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={!!viewingPayment} onOpenChange={(open) => !open && setViewingPayment(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Supplier Payment</DialogTitle>
            <DialogDescription>
              {viewingPayment?.suppliers?.name || viewingPayment?.contact_name || "Supplier payment"}
            </DialogDescription>
          </DialogHeader>
          {viewingPayment && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{format(new Date(viewingPayment.date), "dd MMM yyyy")}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Amount</p>
                <p className="font-medium">{formatKES(Number(viewingPayment.amount))}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Paid from</p>
                <p>{viewingPayment.bank_accounts?.name || "—"}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Reference</p>
                <p>{viewingPayment.reference || "—"}</p>
              </div>
              <div className="col-span-2">
                <p className="text-muted-foreground">Purchase</p>
                <p>{viewingPayment.purchases?.invoice_number || "Unallocated payment"}</p>
              </div>
              <div className="col-span-2">
                <p className="text-muted-foreground">Notes</p>
                <p>{viewingPayment.description || "—"}</p>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <AlertDialog open={!!paymentToDelete} onOpenChange={(open) => !open && setPaymentToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The paid account balance will be restored and linked purchase totals will be recalculated.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                if (paymentToDelete) removePayment.mutate(paymentToDelete);
