    ...
    <div className="w-full min-w-0 space-y-5 overflow-x-hidden">
      ...
      <Card className="p-4 sm:p-5 bg-card border-border shadow-none">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-500" /><h3 className="text-sm font-semibold">Needs attention</h3></div>
          <Link to="/super-admin/activity" className="text-xs text-muted-foreground hover:text-foreground">View audit log →</Link>
        </div>
...
function AttentionCard({ label, value, icon: Icon, link }: { label: string; value: number; icon: any; link: string }) {
  return <Link to={link} className="rounded-lg border p-3 hover:bg-muted/30 transition-colors"><div className="flex items-center justify-between"><Icon className="h-4 w-4 text-muted-foreground" /><span className={`text-lg font-bold ${value > 0 ? "text-amber-600" : ""}`}>{value}</span></div><div className="text-[11px] text-muted-foreground mt-2">{label}</div></Link>;
}
