...
function createSupabaseAdminClient() {
  ...
  if (
    ...
    isPlaceholderSupabaseSecret(SUPABASE_SERVICE_ROLE_KEY)
  ) {
    const missing = [
      ...(!SUPABASE_URL || isPlaceholderSupabaseSecret(SUPABASE_URL) ? ["SUPABASE_URL"] : []),
      ...(!SUPABASE_SERVICE_ROLE_KEY || isPlaceholderSupabaseSecret(SUPABASE_SERVICE_ROLE_KEY)
        ? ["SUPABASE_SERVICE_ROLE_KEY"]
        : []),
    ];
    const message = `Missing Supabase environment variable(s): ${missing.join(", ")}. Add a valid service-role key and project URL.`;
