...
export function BusinessProfileTab() {
  ...
  const { hasFeatureKey } = useEntitlement();
  ...
  const [preventOverselling, setPreventOverselling] = useState(
    (business as { prevent_overselling?: boolean })?.prevent_overselling ?? false,
  );
  const [requireManagerToRemove, setRequireManagerToRemove] = useState(
    (business as { pos_require_manager_to_remove_item?: boolean })?.pos_require_manager_to_remove_item ?? false,
  );
  const [managerApproverId, setManagerApproverId] = useState<string>(
    (business as any)?.pos_manager_approver_id || "any",
  );
  const [businessType, setBusinessType] = useState<BusinessType>(
    ((business as { business_type?: BusinessType })?.business_type || "general") as BusinessType,
  );
  const [trackBatches, setTrackBatches] = useState<boolean>(
    (business as { track_batches?: boolean })?.track_batches ?? false,
  );
  const [posShowStockQty, setPosShowStockQty] = useState<boolean>(
    (business as { pos_show_stock_qty?: boolean })?.pos_show_stock_qty ?? true,
  );
  const [posHideZeroStock, setPosHideZeroStock] = useState<boolean>(
    (business as { pos_hide_zero_stock?: boolean })?.pos_hide_zero_stock ?? true,
  );
  const [lockApprovedStockCounts, setLockApprovedStockCounts] = useState<boolean>(
    (business as { lock_approved_stock_counts?: boolean })?.lock_approved_stock_counts ?? true,
  );
  const [autoPrintReceipt, setAutoPrintReceipt] = useState<boolean>(
    (business as { pos_auto_print_receipt?: boolean })?.pos_auto_print_receipt ?? false,
  );

  const [remindUnpaidPurchases, setRemindUnpaidPurchases] = useState<boolean>(
    (business as { reminders_unpaid_purchases?: boolean })?.reminders_unpaid_purchases ?? false,
  );
  const [remindUnpostedExpenses, setRemindUnpostedExpenses] = useState<boolean>(
    (business as { reminders_unposted_expenses?: boolean })?.reminders_unposted_expenses ?? false,
  );
  ...
  const [loyaltyEnabled, setLoyaltyEnabled] = useState<boolean>(
    (business as { loyalty_enabled?: boolean })?.loyalty_enabled ?? false,
  );
  const [loyaltyPointsPerKes, setLoyaltyPointsPerKes] = useState<string>(
    String((business as { loyalty_points_per_kes?: number })?.loyalty_points_per_kes ?? 1),
  );
  const [loyaltyMinRedeem, setLoyaltyMinRedeem] = useState<string>(
    String((business as { loyalty_min_redeem_points?: number })?.loyalty_min_redeem_points ?? 100),
  );
  const [loyaltyMinPurchase, setLoyaltyMinPurchase] = useState<string>(
    String((business as { loyalty_min_purchase_amount?: number })?.loyalty_min_purchase_amount ?? 0),
  );
  const [loyaltyKesPerPoint, setLoyaltyKesPerPoint] = useState<string>(
    String((business as { loyalty_kes_per_point?: number })?.loyalty_kes_per_point ?? 1),
  );
                ...
                <div className="sm:col-span-2 flex items-center justify-between rounded-lg border p-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Track batch numbers and expiry dates on products and inventory.
                    </p>
                  </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Block sales and adjustments that would push stock below zero.
                </p>
                {negativeStockCount > 0 && !preventOverselling && (
                  <p className="text-xs text-destructive mt-1">
                    Cannot enable — {negativeStockCount} product{negativeStockCount > 1 ? "s have" : " has"} negative
                    stock. Reconcile inventory to 0 or above first.
                  </p>
                )}
              </div>
              <Switch
                ...
                onCheckedChange={(v) => {
                  if (v && negativeStockCount > 0) {
                    toast.error(
                      `Cannot enable: ${negativeStockCount} product(s) have negative stock. All stocks must be 0 or above.`,
                    );
                    return;
                  }
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-base flex items-center gap-2">
                  <ShoppingCart className="h-4 w-4" /> Require manager approval to remove a scanned item
                </Label>
                <p className="text-sm text-muted-foreground">
            ...
            {requireManagerToRemove && (
              <div className="space-y-2 pt-2 border-t">
                ...
                <Select value={managerApproverId} onValueChange={setManagerApproverId}>
                  <SelectTrigger>
                    <SelectValue placeholder="Any admin or manager" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <p className="text-xs text-muted-foreground">
                  If set, only this specific user can approve POS item removal. Otherwise any admin or manager can
                  approve.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Display the available quantity badge next to each product in the POS product listing. Turn off to hide
                  stock quantities from cashiers.
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Products with quantity of zero or below at the current location won't appear in the POS product list.
                  Turn off to show every active product regardless of stock.
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-base flex items-center gap-2">
                  <Printer className="h-4 w-4" /> Automatically print receipt on completed sale
                </Label>
                <p className="text-sm text-muted-foreground">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Once a stock take is approved it can no longer be edited or deleted. Turn off to allow admins to amend
            ...
            {loyaltyEnabled && (
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Example: 1 = 1 point per KES 1 spent.</p>
                </div>
                <div className="space-y-2">
