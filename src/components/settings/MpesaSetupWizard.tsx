        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <CardDescription className="mt-1 max-w-2xl">
              Configure your Daraja account, securely store credentials, verify the connection, and optionally connect your incoming SMS modem.
            </CardDescription>
          </div>
            ...
            return (
              <button
                ...
                className={`rounded-lg border p-2 text-left text-xs transition ${
                  active ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30" : complete ? "border-emerald-200" : "border-muted"
                }`}
              >
      ...
      <CardContent className="p-5 md:p-6">
          ...
          <div className="grid gap-5 md:grid-cols-[1.4fr_1fr]">
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                This wizard takes you through the minimum configuration required for STK Push payments. Your Daraja secrets are encrypted and are never displayed after they are saved.
              </p>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground">These values are safe to store in the business record and are used when creating STK Push requests.</p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={props.shortcode} onChange={(e) => props.setShortcode(e.target.value.replace(/\D/g, ""))} placeholder="e.g. 174379" inputMode="numeric" />
              </div>
              <div className="space-y-2">
                <Label>Account reference</Label>
                <Input value={props.accountReference} onChange={(e) => props.setAccountReference(e.target.value)} placeholder="e.g. STRATUS" />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label>Callback URL</Label>
                <Input value={props.callbackUrl} onChange={(e) => props.setCallbackUrl(e.target.value)} placeholder="https://<project>.supabase.co/functions/v1/mpesa-callback?type=stk" />
        ...
        {step === 2 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Paste the credentials from your Daraja app. They are sent to the secure server function and stored in Vault.</p>
            </div>
            {props.hasCreds && (
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input type={showSecrets ? "text" : "password"} value={props.consumerKey} onChange={(e) => props.setConsumerKey(e.target.value)} placeholder={props.hasCreds ? "Leave blank to keep current" : "Paste Consumer Key"} />
              </div>
              <div className="space-y-2">
                <Label>Consumer Secret</Label>
                <Input type={showSecrets ? "text" : "password"} value={props.consumerSecret} onChange={(e) => props.setConsumerSecret(e.target.value)} placeholder={props.hasCreds ? "Leave blank to keep current" : "Paste Consumer Secret"} />
              </div>
              <div className="space-y-2">
                <Label>Lipa na M-Pesa Passkey</Label>
                <Input type={showSecrets ? "text" : "password"} value={props.passkey} onChange={(e) => props.setPasskey(e.target.value)} placeholder={props.hasCreds ? "Leave blank to keep current" : "Paste Passkey"} />
              </div>
        ...
        {step === 3 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground">We'll request a short-lived OAuth access token. No payment is initiated.</p>
            </div>
            <div className="rounded-xl border bg-muted/30 p-5">
              <div className="grid gap-4 sm:grid-cols-3">
                <div><div className="text-xs text-muted-foreground">Environment</div><div className="mt-1 font-medium">{props.environment === "live" ? "Live" : "Sandbox"}</div></div>
                <div><div className="text-xs text-muted-foreground">Shortcode</div><div className="mt-1 font-medium">{props.shortcode || "Not set"}</div></div>
                <div><div className="text-xs text-muted-foreground">Credentials</div><div className="mt-1 font-medium">{props.hasCreds ? "Saved securely" : "Not saved"}</div></div>
              </div>
            </div>
            ...
            {testPassed && <div className="flex items-center gap-2 text-sm text-emerald-700"><Check className="h-4 w-4" /> Connection verified successfully.</div>}
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Optional. This lets the POS receive M-Pesa confirmation SMS messages and manually or automatically match them to sales.</p>
            </div>
            <div className="rounded-xl border p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="font-medium">Modem ingestion</div>
                  <div className="text-sm text-muted-foreground">{props.smsTokenConfigured ? `Configured (${props.smsTokenPrefix ?? "token"}••••)` : "Not configured"}</div>
                </div>
                <Badge variant={props.smsTokenConfigured ? "default" : "secondary"}>{props.smsTokenConfigured ? "Ready" : "Optional"}</Badge>
              </div>
              {props.smsToken && (
                <div className="mt-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" onClick={() => void copyToken()}><Copy className="h-4 w-4" /></Button>
                  </div>
                  <p className="text-xs text-muted-foreground">This secret is shown once. Store it in the modem integration.</p>
                </div>
              )}
              <div className="mt-4 flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => void props.onGenerateSmsToken()} disabled={props.smsTokenBusy}>
                  {props.smsTokenBusy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <MessageSquareText className="mr-2 h-4 w-4" />}
                  {props.smsTokenConfigured ? "Rotate token" : "Generate modem token"}
                </Button>
                {props.smsTokenConfigured && <Button variant="destructive" onClick={() => void props.onRevokeSmsToken()} disabled={props.smsTokenBusy}>Revoke token</Button>}
              </div>
            </div>
            <div className="rounded-lg bg-muted/40 p-3 text-xs font-mono break-all">
              POST /functions/v1/mpesa-sms-ingest<br />
              Header: x-mpesa-ingest-token: YOUR_TOKEN<br />
              JSON: {'{ business_id, message, sender, received_at }'}
            </div>
            <p className="text-xs text-muted-foreground">You can skip this step and configure the modem later from the M-Pesa settings page.</p>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="mx-auto max-w-xl py-6 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Your business configuration is saved. You can now use M-Pesa from POS, monitor transactions, and reconcile incoming SMS payments.</p>
            <div className="mt-5 grid gap-2 text-left sm:grid-cols-2">
              {["Business configuration saved", "Daraja credentials secured", "Connection verified", props.smsTokenConfigured ? "SMS modem connected" : "SMS modem can be added later"].map((item) => (
                <div key={item} className="flex items-center gap-2 rounded-lg border p-3 text-sm"><Check className="h-4 w-4 text-emerald-600" />{item}</div>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          {step === 0 && <Button onClick={next}>Start setup <ChevronRight className="ml-1 h-4 w-4" /></Button>}
          {step === 1 && <Button onClick={() => void saveBusinessSetup()} disabled={props.savingPublic}>{props.savingPublic && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save & continue <ChevronRight className="ml-1 h-4 w-4" /></Button>}
          {step === 2 && <Button onClick={() => void saveCredentials()} disabled={props.savingSecrets}>{props.savingSecrets && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{props.hasCreds && !props.consumerKey ? "Keep credentials & continue" : "Secure credentials & continue"}<ChevronRight className="ml-1 h-4 w-4" /></Button>}
          {step === 3 && <Button onClick={next} disabled={!testPassed}>Continue <ChevronRight className="ml-1 h-4 w-4" /></Button>}
          {step === 4 && <div className="flex gap-2 sm:ml-auto"><Button variant="outline" onClick={next}>Skip for now</Button><Button onClick={next}>Continue <ChevronRight className="ml-1 h-4 w-4" /></Button></div>}
          {step === 5 && <Button onClick={props.onFinish}>Finish setup</Button>}
        </div>
      </CardContent>
    </Card>
  );
