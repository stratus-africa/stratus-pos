...
export function PaymentGatewaysTab() {
  ...
  const testStkPush = async (phoneNumber: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error || "STK Push test failed");
      toast.success("STK Push prompt sent", { description: "Complete the KES 1 prompt on the test phone." });

      const checkoutRequestId = data.checkoutRequestId;
      for (let i = 0; i < 12; i++) {
        await new Promise((resolve) => setTimeout(resolve, 5000));
        const poll = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/mpesa?action=stk-test-query`, {
          method: "POST",
