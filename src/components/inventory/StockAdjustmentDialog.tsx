...
export function StockAdjustmentDialog({ open, onOpenChange, onSubmit, isLoading }: Props) {
  ...
  const addProduct = (product: Product, qty: number = 1) => {
    setLines((prev) => {
      ...
      if (existing) {
        return prev.map((l) => (l.product_id === product.id ? { ...l, quantity_change: l.quantity_change + qty } : l));
      }
      return [
        ...prev,
        {
  ...
  const handleQuantityChange = (productId: string, qty: number) => {
    setLines((prev) => prev.map((l) => (l.product_id === productId ? { ...l, quantity_change: qty } : l)));
  };

  const handleUnitCostChange = (productId: string, cost: number) => {
    setLines((prev) => prev.map((l) => (l.product_id === productId ? { ...l, unit_cost: cost } : l)));
  };
                ...
                <Table>
                  ...
                  <TableBody>
                    {lines.map((l) => {
                      ...
                      const inputValue = isPurchase ? l.quantity_change : current + (Number(l.quantity_change) || 0); // new stock on hand
                        ...
                        <TableRow key={l.product_id}>
                          ...
                          <TableCell>
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8"
                              onClick={() => handleRemoveLine(l.product_id)}
                            >
              ...
              <div className="flex flex-col text-sm text-muted-foreground">
                ...
                {draftSavedAt && (
                  <span className="text-xs flex items-center gap-1">
                    <FileText className="h-3 w-3" /> Draft saved {new Date(draftSavedAt).toLocaleString()}
                  </span>
                )}
              </div>
